...
export function Play() {
  ...
  const [gameSessionId, setGameSessionId] = useState(
    useSelector(selectGameSessionID)
  ); // TODO: get gameSessionId from redux store
  const [userSessionId, setUserSessionId] = useState(""); // TODO: get userSession from redux store
  const [isHost, setIsHost] = useState(useSelector(selectIsHost));
    ...
    <>
      ...
      {!isHost && (
        <Flex direction="row" justifyContent="center">
          ...
          ></TextField>
          {!isHost && (
            <ToggleButton color="#FF6DDF" onClick={handleUserResponse}>
              Submit
            </ToggleButton>
          )}
        </Flex>
