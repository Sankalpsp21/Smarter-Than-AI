...
import { setIsHost, setGameSessionID } from '../redux/GameSlice';
	...
	const handleStartbtn = async () => {
		try {
			...
			if (gameSession == null || gameSession.roundNumber > 0) {
				console.error(
					'Invalid pin code or game session has already started'
				);
				setIsError(true);
				setErrorMessage(
					'Invalid pin code or game session has already started'
	...
	return (
		<>
			...
			<Flex direction="row">
				...
				<Button
					onClick={async () => {
						let result = await copyToClipboard({
							target: '.pincode'
						});
